...
namespace WeifenLuo.WinFormsUI.Docking
{
    partial class DockPanel
    {
        private static class Persistor
        {
            ...
            public static void SaveAsXmlRemark(DockPanel dockPanel, string fileName, Encoding encoding)
            {
                using (var fs = new FileStream(fileName, FileMode.Create))
                {
                    try
                    {
                        SaveAsXmlRemark(dockPanel, fs, encoding, false);
                    }
                    finally
                    {
                        fs.Close();
            ...
            public static void SaveAsXmlRemark(DockPanel dockPanel, Stream stream, Encoding encoding, bool upstream)
            {
                ...
                foreach (IDockContent content in dockPanel.Contents)
                {
                    ...
                    IDockRemark remark = content as IDockRemark;
                    if (remark != null)
                    {
                        xmlOut.WriteAttributeString("PersistString", content.DockHandler.PersistString + "," + remark.Remark);         
                    }
                    else
                    {
        ...
        public void SaveAsXmlRemark(string fileName, Encoding encoding)
        {
            Persistor.SaveAsXmlRemark(this, fileName, encoding);
        }
